...
.sidebar-nav-active {{
    background: {"rgba(37,99,235,0.15)" if dark else "rgba(37,99,235,0.08)"} !important;
    color: {PRIMARY} !important;
    font-weight: 600;
}}
.sidebar-nav-active * {{ color: {PRIMARY} !important; -webkit-text-fill-color: {PRIMARY} !important; }}
...
with st.sidebar:
    ...
    for icon, label, key in nav_items:
        active_class = "sidebar-nav-active" if st.session_state.active_tab == key else ""
        st.markdown(f"""
            ...
            with st.spinner("Indexing document..."):
                ...
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        """, unsafe_allow_html=True)
                        st.markdown(f'<div class="stat-badge">📦 {chunks} chunks stored</div>', unsafe_allow_html=True)
                    else:
...
with col1:
    ...
    <div class="metric-card">
        ...
        <div class="metric-label">Instant AI Analysis</div>
    </div>
    """, unsafe_allow_html=True)
with col2:
    st.markdown(f"""
    <div class="metric-card">
        <div class="metric-value">🔍</div>
        <div class="metric-label">Clause Extraction</div>
    </div>
    """, unsafe_allow_html=True)
with col3:
    st.markdown(f"""
    <div class="metric-card">
        <div class="metric-value">📄</div>
        <div class="metric-label">Smart Summaries</div>
    </div>
    """, unsafe_allow_html=True)
...
with tab1:
    ...
    user_query = st.text_input(
        "YOUR QUESTION",
        placeholder="e.g. Is there a waiting period for pre-existing diseases?",
    ...
    if st.button("✨ Analyze with AI", key="analyze_btn", use_container_width=True):
        if user_query:
            ...
            with st.spinner("🧠 AI is thinking..."):
                try:
                    res = requests.post(f"{API_BASE_URL}/ask-question", json={"query": user_query})
...
with tab2:
    ...
    if st.button("✨ Auto-Extract Clauses", key="extract_btn", use_container_width=True):
        with st.spinner("🔬 Scanning document for structured data..."):
            try:
                res = requests.get(f"{API_BASE_URL}/extract-clauses")
                if res.status_code == 200:
                    st.session_state.clauses_data = res.json()
                    st.rerun()
                else:
                    st.error("Could not extract clauses. Please upload a document first.")
            except requests.exceptions.ConnectionError:
                st.error("🔌 Backend offline. Please start the FastAPI server.")
            except Exception as e:
                st.error(f"Error: {e}")
    ...
    if st.session_state.clauses_data:
        ...
        with col1:
            st.markdown(f"""
            <div class="clause-card clause-coverage fade-in-up">
                <div class="clause-label clause-label-green">✅ Coverage</div>
                <div style="font-size:0.92rem; line-height:1.7;">{data.get("Coverage", "N/A")}</div>
            </div>
            """, unsafe_allow_html=True)
        
        with col2:
            st.markdown(f"""
            <div class="clause-card clause-exclusion fade-in-up">
                <div class="clause-label clause-label-red">⚠️ Exclusions</div>
                <div style="font-size:0.92rem; line-height:1.7;">{data.get("Exclusions", "N/A")}</div>
            </div>
            """, unsafe_allow_html=True)
        
        st.markdown(f"""
        <div class="clause-card clause-premium fade-in-up">
            <div class="clause-label clause-label-amber">💰 Premium & Deductibles</div>
            <div style="font-size:0.92rem; line-height:1.7;">{data.get("Premium", "N/A")}</div>
        </div>
        """, unsafe_allow_html=True)
    else:
        st.markdown("""
        <div class="empty-state fade-in">
            <div class="empty-state-icon">📋</div>
            <div class="empty-state-text">No clauses extracted yet.<br/>Click the button above to begin extraction.</div>
        </div>
        """, unsafe_allow_html=True)
...
with tab3:
    ...
    if st.button("📄 Generate Summary", key="summary_btn", use_container_width=True):
        with st.spinner("📊 Synthesizing comprehensive summary..."):
            try:
                res = requests.get(f"{API_BASE_URL}/summarize")
                if res.status_code == 200:
                    st.session_state.summary_data = res.json().get("summary", "")
                    st.rerun()
                else:
                    st.error("Summary generation failed. Make sure documents are uploaded.")
            except requests.exceptions.ConnectionError:
                st.error("🔌 Backend offline. Please start the FastAPI server.")
            except Exception as e:
                st.error(f"Error: {e}")
    ...
    if st.session_state.summary_data:
        st.markdown(f"""
        <div class="solid-card fade-in-up" style="border-left: 4px solid #10B981;">
            <div class="clause-label clause-label-green">📖 Policy Summary</div>
        ...
        <div class="empty-state fade-in">
            <div class="empty-state-icon">📝</div>
            <div class="empty-state-text">No summary generated yet.<br/>Click the button above to create one.</div>
        </div>
        """, unsafe_allow_html=True)
...
st.markdown("""
<div class="footer">
    Powered by <strong>Antigravity RAG</strong> · Built for PolicyBazaar Document Analysis<br/>
    <span style="opacity: 0.5; font-size: 0.75rem;">© 2026 PolicyBazaar AI. All rights reserved.</span>
</div>
""", unsafe_allow_html=True)
